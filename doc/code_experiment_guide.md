...
- `latent_out_proj` 用于把 transition 某个位置的 hidden state 投回 decoder 可接受的 latent 空间

当前实现口径固定为：

- 因为当前实验里已经把 `latent_dim` 和 GPT-2 hidden size 统一对齐到 `768`
- `latent_in_proj` 使用 identity init
- `latent_out_proj` 使用 identity init
...
也就是先让 transition 输出接口从近似恒等映射开始，再由训练去学习更合适的 latent-space 对齐。
...
`reencode` 相关指标保留在详细输出里，主要用于辅助排查 codec 和 transition 串接后的误差积累，不作为当前接口判断的主指标。

另外，transition 当前还支持一个可选的 latent 辅助监督：
...
- `pred_vs_target_latent_cosine` 越高越好，表示 predicted latent 和目标 encoder latent 更接近
- `pred_vs_target_latent_mse` 越低越好，表示 predicted latent 和目标 encoder latent 的几何距离更小
...
这两个文件现在都整理成一个紧凑概览对象，而不是单纯的 epoch 列表。默认包含：

- `latest`：最后一个 epoch 的关键指标
- `best_by_loss`：按 valid loss 最优的 epoch
- `best_by_metric`：按不同关键指标自动找出的最佳 epoch
- `recent_epochs`：最近 5 个 epoch 的紧凑摘要

这样复制给外部模型分析时，既不会太长，也不需要先手工挑出哪几个 epoch 最值得看。

训练配置现在还支持：

- `train.grad_accum_steps`

它表示每累计多少个 micro-batches 再做一次 `optimizer.step()`。因此有效 batch size 约等于：

```text
effective_batch_size ~= data.batch_size * train.grad_accum_steps * world_size
```

这对 `gpt2-small` 和 `gpt2-xl` 切换时尤其有用，因为可以在显存受限时先减小单卡 batch，再用梯度累计把有效 batch 补回来。
...
## 12. ProsQA 的 SFT sanity check

为了判断 ProsQA 这个任务对当前 GPT-2 模型来说是否本身就难，当前额外实现了三种 SFT baseline：

1. `next_step`
   - 输入：`question + previous_steps`
   - 输出：`current_step`
   - 对应目的：检查不用 latent 接口时，普通条件语言模型能否学会下一步推理

2. `answer_from_steps`
   - 输入：`question + all_steps`
   - 输出：`answer`
   - 对应目的：检查给出完整证明轨迹后，模型能否稳定输出最终答案

3. `answer_from_question`
   - 输入：`question`
   - 输出：`answer`
   - 对应目的：检查不显式给中间步骤时，模型最终问答的直接上限
...
- `scripts/train_sft_prosqa_next_step_ntp.sh`
- `scripts/train_sft_prosqa_next_step_mtp.sh`
- `scripts/train_sft_prosqa_answer_from_steps_ntp.sh`
- `scripts/train_sft_prosqa_answer_from_steps_mtp.sh`
- `scripts/train_sft_prosqa_answer_from_question_ntp.sh`
- `scripts/train_sft_prosqa_answer_from_question_mtp.sh`
...
- `sft_valid_generations/epoch_xxx.json`
